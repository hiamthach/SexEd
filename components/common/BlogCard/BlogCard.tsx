...
const BlogCard = ({ title, subTitle, coverImg, hashtags, id }: IBlog) => {
  ...
  return (
    <Card shadow="sm" p="lg" radius="lg" withBorder className="hover:shadow-md cursor-pointer w-full h-full">
      <Card.Section>
        <Image src={coverImg} height={256} alt="Norway" />
      </Card.Section>
      <div className="px-[6px] pt-[24px] pb-[16px] text-left flex flex-col gap-1 justify-end">
        <h3 className="font-heading text-[20px] leading-[24px] text-[#171A1FFF] line-clamp-3" title={title}>
          {title}
        </h3>
